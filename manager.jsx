...
function MgrOverview(){
  ...
  return h('div',{className:'page'},
    ...
    h('div',{className:'card'},
      h('div',{className:'card-head-row', style:{marginBottom:18}},
        h('div',null,
          h('div',{className:'card-title'},'Where the 5,400 Stand'),
          h('div',{className:'card-sub'},'Distribution of all employees across progression tiers.')),
        h('div',{className:'filter-note'}, h('b',null,'3,078'),' at Gold or above')),
      h('div',{className:'dist'},
        M.distribution.map((d,i)=>{
          const t=tierByKey(d.tier); const pct=(d.count/M.total*100);
          return h('div',{className:'row',key:i},
            h('div',{className:'name'}, h('span',{className:'swh',style:{background:t.color}}), t.name),
            h('div',{className:'track'}, h('i',{style:{width:pct+'%', background:`linear-gradient(90deg,${t.color}cc,${t.color})`}})),
            h('div',{className:'cnt'}, d.count.toLocaleString(),' ', h('span',null,`(${pct.toFixed(0)}%)`)));
        }))
    )
  );
}
