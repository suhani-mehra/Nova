...
function mapMe(data) {
  return {
    ...
    av:    data.role === 'manager' ? ['#2ACCFF', '#5400DC'] : ['#A634FF', '#FF4398'],
    kind:  data.role,
  };
}

// ── Boot ──────────────────────────────────────────────────────────────────────
// api.js resolves this Promise once data is ready; app.jsx awaits it before
// calling ReactDOM.createRoot().render() so the first paint has real data.
...
async function initNova() {
  try {
    ...
    // Only overwrite the current user's slot; the other stays as dummy data
